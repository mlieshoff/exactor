...
public class TestCompareDynamicFiles extends TestCase {
    ...
    public void testCompareFiles_DoNotMatch() throws Exception {
        ...
        compareFiles.setScript(new Script(new File(Constants.pathToTestFile("dummy.act"))));
        compareFiles.addParameter(new Parameter(Constants.pathToTestFile("comparedynamicfiles/expected_file.txt")));
        compareFiles.addParameter(new Parameter(Constants.pathToTestFile(
                "comparedynamicfiles/actual_file_not_matching.txt")));
        compareFiles.addParameter(new Parameter("1"));
        try {
            compareFiles.execute();
        } catch (AssertionFailedError e) {
            assertEquals("Content of expected file: '" + Constants.pathToTestFile(
                    "comparedynamicfiles/expected_file.txt") + "' does not match actual file: '"
                    + Constants.pathToTestFile("comparedynamicfiles/actual_file_not_matching.txt")
                    + "' expected:<...ghi...> but was:<...xxx...>", e.getMessage());
        }
    }

    public void testCompareFilesRelative_Match() throws Exception {
        CompareDynamicFiles compareFiles = new CompareDynamicFiles();
        compareFiles.setScript(new Script(new File(Constants.pathToTestFile(
                "comparedynamicfiles/script_for_relative_path_test.act"))));
