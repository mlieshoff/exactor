...
public class AbbotToExactorConverter {
    ...
    private static void processFile(File file, boolean guessAlias) throws Exception {
        String exactorFileName = extractFileParentDir(file) + File.separator + extractFileMinusExtension(file) + ".act";
        System.out.println("Processing Abbot script " + file.getAbsolutePath() + " into Exactor script "
                + exactorFileName + " ...");
        ...
        fileWriter.close();
        System.out.println("Processed Abbot script " + file.getAbsolutePath() + " into Exactor script "
                + exactorFileName);
    }
    ...
    static String processElement(Element element, boolean guessAlias) throws Exception {
        ...
        if (element.getName().equals("action")) {
            if (guessAlias && !(checkForAnAlias(element).equals(""))) {
                ...
                result += AbbotToExactorConverter.convertAbbotXmlToExactorScript(element, "AbbotAction",
                        new String[]{"class", "method", "args"});
            }
        } else if (element.getName().equals("assert")) {
            result += AbbotToExactorConverter.convertAbbotXmlToExactorScript(element, "AbbotAssert",
                    new String[]{"class", "method", "args", "value", "component", "invert"});
        } else if (element.getName().equals("wait")) {
            result += AbbotToExactorConverter.convertAbbotXmlToExactorScript(element, "AbbotWait",
                    new String[]{"class", "method", "args", "invert", "component", "timeout", "value"});
        } else if (element.getName().equals("launch")) {
            element.setAttribute("classpath", element.getAttribute("classpath").getValue().replace('\\', '/'));
            result += AbbotToExactorConverter.convertAbbotXmlToExactorScript(element, "AbbotLaunch",
                    new String[]{"class", "method", "args", "classpath"});
        } else if (element.getName().equals("component")) {
            result += AbbotToExactorConverter.convertAbbotXmlToExactorScript(element, "AbbotComponent",
                    new String[]{"class", "id", "name", "index", "parent", "window", "root", "title", "tag", "icon",
                            "invoker", "borderTitle"});
        } else if (element.getName().equals("terminate")) {
            result += AbbotToExactorConverter.convertAbbotXmlToExactorScript(element, "AbbotTerminate",
                    new String[]{});
        } else if (element.getName().equals("appletviewer")) {
            result += AbbotToExactorConverter.convertAbbotXmlToExactorScript(element, "AbbotAppletViewer",
                    new String[]{"archive", "code", "height", "width"});
        } else if (element.getName().equals("annotation")) {
            String converted = AbbotToExactorConverter.convertAbbotXmlToExactorScript(element, "AbbotAnnotation",
                    new String[]{"component", "userDismiss", "x", "y", "height", "width", "desc"});
            ...
            result += AbbotToExactorConverter.convertAbbotXmlToExactorScript(element, "AbbotEvent",
                    new String[]{"component", "kind", "type", "x", "y", "keyCode", "modifiers"});
        } else {
