...
public class AbbotToExactorConverterTest extends TestCase
{
    ...
    {
        File[] abbotFiles = FileCollector.filesWithExtension( new File( Constants.DATA_DIR ), "xml" );
        File[] expectedExactorFiles = FileCollector.filesWithExtension( new File( Constants.DATA_DIR ), "act" );

        for( int i = 0; i < abbotFiles.length; i++ )
        {
            String expected = Util.getFileContent( expectedExactorFiles[i].getAbsolutePath() );

            assertEquals( "Conversion of : " + abbotFiles[i].getName() + " did not match content in " + expectedExactorFiles[i].getName(),
                    expected.trim(),
                    AbbotToExactorConverter.convertToExactor( abbotFiles[i], true ).trim() );
        }
    }

    public void testConvertAbbotComponentXMLToExactorScript() throws Exception
    {
        assertComponentXMLToExactorScript( "AbbotComponent \"CLASS\" \"ID\" \"NAME\" \"INDEX\" \"PARENT\" \"WINDOW\" \"ROOT\" \"TITLE\" \"TAG\" \"ICON\" \"INVOKER\" \"BORDERTITLE\"",
    ...
    {
        assertComponentXMLToExactorScript( "AbbotEvent \"COMPONENT\" \"KIND\" \"TYPE\" \"X\" \"Y\" \"KEYCODE\" \"MODIFIERS\"",
    ...
    public void testConvertAbbotLaunchXMLToExactorScript() throws Exception
    {
        assertEquals( "AbbotLaunch \"com.exoftware.exactor.ide.ExactorIDE\" \"main\" \"[]\" \".\"",
                AbbotToExactorConverter.processElement( Util.createElement( "<launch args=\"[]\" class=\"com.exoftware.exactor.ide.ExactorIDE\" classpath=\".\" method=\"main\" />" ), false ) );
    }
    ...
    public void testConvertAbbotLaunchXMLToExactorScript_ChangeBackSlashesToForward() throws Exception
    {
        assertEquals( "AbbotLaunch \"com.exoftware.exactor.ide.ExactorIDE\" \"main\" \"[]\" \"c:/adir/ajar.jar\"",
                AbbotToExactorConverter.processElement( Util.createElement( "<launch args=\"[]\" class=\"com.exoftware.exactor.ide.ExactorIDE\" classpath=\"c:\\adir\\ajar.jar\" method=\"main\" />" ), false ) );
    }
    ...
    {
        assertEquals( expectedExactorScript,
                AbbotToExactorConverter.processElement( Util.createElement( actionXML ), false ) );
    }

    private void assertComponentXMLToExactorScript( String expectedExactorScript, String compXML ) throws Exception
    {
        assertEquals( expectedExactorScript,
                AbbotToExactorConverter.processElement( Util.createElement( compXML ), false ) );
    }
}
